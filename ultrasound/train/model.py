...
class SignalReduce(nn.Module):
    ...
    def __init__(self):
        ...
        self.fn_out = nn.Sequential(nn.Linear(SIG_OUT_SIZE, EMB_SIZE))
...
class SignalAtt(nn.Module):
    ...
    def __init__(self):
        ...
        enc_layer = nn.TransformerEncoderLayer(
            EMB_SIZE, nhead=NUM_HEADS, dim_feedforward=DFF, batch_first=True
        )
...
class UpsampleLayer(nn.Module):
    def __init__(
        self, in_ch, out_ch, out_size=None, scale_factor=2, residual_layers=True
    ):
        super().__init__()
        # if residual_layers:
        if out_size is None:
